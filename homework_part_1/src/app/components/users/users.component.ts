...
export class UsersComponent implements OnInit {
  ...
  public printAll(dataSource:any[]){
    ...
    dataSource!.forEach((element:any) => {
      ...
      pSName.textContent=element.secondname;
  ...
  private sortingChange(type:string){
    ...
    switch(type){
      case "none":
        ...
        break;
      case "username":
        inpSelector?.value===""?this.printAll(this.dataSource.sort((a, b) => (a.firstname > b.firstname) ? 1 : -1)):this.arrayForSearch.sort((a, b) => (a.firstname > b.firstname) ? 1 : -1);
        break;
      case "lastname":
        inpSelector?.value===""?this.printAll(this.dataSource.sort((a, b) => (a.secondname > b.secondname) ? 1 : -1)):this.arrayForSearch.sort((a, b) => (a.secondname > b.secondname) ? 1 : -1);
        break;
    }
  }
  //-----------------------------------
  ...
  private searching(type:string|undefined|null){
    ...
    if(type!==""){
      this.arrayForSearch=this.dataSource.filter(((el:any)=>el.firstname===type||el.secondname===type));
      this.printAll(this.arrayForSearch);
    }
    else{
      this.printAll(this.dataSource);
